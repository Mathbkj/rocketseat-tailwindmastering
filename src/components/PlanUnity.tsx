...
const Checked: FC = () => (
  <Check size={25} color="white" className="bg-violet-800 rounded-full" />
);
...
export const PlanUnity: FC<Props> = ({ title, cost }) => {
  return (
    <Wrapper
      variant="ghost"
      className="flex group hover:scale-105 relative items-center flex-col px-8"
    >
      <div className="flex flex-col gap-4">
        <span className="text-sm! text-zinc-800 dark:text-zinc-400">
          {title}
        </span>
        <span className="text-lg! text-zinc-900 dark:text-zinc-200">
          ${cost}
        </span>
      </div>
